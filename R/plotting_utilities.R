...
draw_tennis_court_lines_ggplot <- function(){
  ...
  mid_line <- data.frame(x = c(-service_line, service_line),
                         y = c(0, 0),
                         ID = rep('mid_line', 2))
  
  top_sideline <- data.frame(x = c(-baseline_x, baseline_x),
                             y = rep(baseline_y, 2),
                             ID = rep('top_sideline', 2))
  
  bottom_sideline <- data.frame(x = c(-baseline_x, baseline_x),
                                y = rep(-baseline_y, 2),
                                ID = rep('bottom_sideline', 2))
  
  far_top_sideline <- data.frame(x = c(-baseline_x, baseline_x),
                                 y = rep(baseline_y + doubles_alley_width, 2),
                                 ID = rep('far_top_sideline', 2))
  
  far_bottom_sideline <- data.frame(x = c(-baseline_x, baseline_x),
                                    y = rep(-baseline_y - doubles_alley_width, 2),
                                    ID = rep('far_bottom_sideline', 2))
  
  pos_baseline <- data.frame(x = rep(baseline_x, 2),
                             y = c(-baseline_y - doubles_alley_width, baseline_y + doubles_alley_width),
                             ID = rep('pos_baseline', 2))
  
  neg_baseline <- data.frame(x = rep(-baseline_x, 2),
                             y = c(-baseline_y - doubles_alley_width, baseline_y + doubles_alley_width),
                             ID = rep('neg_baseline', 2))
  
  pos_service_line <- data.frame(x = rep(service_line, 2),
                                 y = c(-baseline_y, baseline_y),
                                 ID = rep('pos_service_line', 2))
  
  neg_service_line <- data.frame(x = rep(-service_line, 2),
                                 y = c(-baseline_y, baseline_y),
                                 ID = rep('neg_service_line', 2))
  
  df <- rbind(mid_line, top_sideline, bottom_sideline, far_top_sideline, far_bottom_sideline,
              pos_baseline, neg_baseline, pos_service_line, neg_service_line)
...
draw_tennis_court_surface_ggplot <- function(){
  ...
  ob_surface <- data.frame(xmin = -max_x,
                           xmax = max_x,
                           ymin = -max_y,
                           ymax = max_y,
                           ID = 'ob_surface')
  
  ib_surface <- data.frame(xmin = -baseline_x,
                           xmax = baseline_x,
...
draw_region_lines_ggplot <- function(){
  ...
  df <- mutate(data.frame(x = c(rep(ver_lines, each = 2),
                          ...
                          y = c(rep(c(-max_y, max_y), length = 2*length(ver_lines)),
...
draw_region_numbers_ggplot <- function(){
  ...
  net_to_baseline_inds <- sapply(net_to_baseline_regions,
                                 function(x) which(region_grid == x, arr.ind = TRUE)) %>%
    t()
  
  net_to_baseline_locs <- apply(net_to_baseline_inds,
                                1,
                                function(x) c(mean(c(pos_ver_lines[x[1]],
                                                     pos_ver_lines[x[1]+1])),
                                              mean(c(hor_lines_general_rev[x[2]],
                                                     hor_lines_general_rev[x[2] + 1])))) %>%
    t()
  ...
  behind_baseline_inds <- sapply(behind_baseline_regions,
                                 function(x) which(region_grid == x, arr.ind = TRUE)) %>%
    t()
  behind_baseline_locs <- apply(behind_baseline_inds,
                                1,
                                function(x) c(mean(c(pos_ver_lines[x[1]],
                                                     pos_ver_lines[x[1]+1])),
                                              mean(c(hor_lines_general_rev[x[2]],
  ...
  baseline_inds <- sapply(baseline_regions,
                          function(x) which(baseline_regions == x)) %>%
    t()
  baseline_locs <- sapply(baseline_inds,
                          function(x) c(mean(c(pos_ver_lines[3],
                                               pos_ver_lines[4])) + 0.75,
                                        mean(c(hor_lines_baseline_rev[x],
                                               hor_lines_baseline_rev[x+1])))) %>%
    t()
